...
public:
	...
	void preorderTraversal( void ( *visit ) ( elemType& item ) ) const;
	// Function to do a preorder traversal of the binary tree.
	// Postcondition: Nodes are printed in preorder sequence.

	void postorderTraversal( void ( *visit ) ( elemType& item ) ) const;
...
private:
	...
	void inorder( nodeType<elemType>* p, void ( *visit ) ( elemType& item ) ) const;
	// Function to do an inorder traversal of the binary
	// tree to which p points.
	// Postcondition: Nodes of the binary tree, to which p
	//               points, are printed in inorder sequence.

	void preorder( nodeType<elemType>* p, void ( *visit ) ( elemType& item ) ) const;
	...
	void postorder( nodeType<elemType>* p, void ( *visit ) ( elemType& item ) ) const;
...
const binaryTreeType<elemType>& binaryTreeType<elemType>::operator=(
	...
	if ( this != &otherTree ) { // avoid self-copy
		...
		if ( this->root != nullptr )  // if the binary tree is not empty, destroy the binary tree
			destroy( root );
...
template <class elemType>
int binaryTreeType<elemType>::nodeCount( nodeType<elemType>* node ) const {
	if ( !node ) // no vaid node return 0 ending recursion
		return 0;
	else // use recursion to add trees til no more nodes (+1 for root node)
		return nodeCount( node->rLink ) + nodeCount( node->lLink ) + 1;
}

template <class elemType>
int binaryTreeType<elemType>::leavesCount( nodeType<elemType>* p ) const {
	if ( !p ) // no vaid node return 0 ending recursion
		return 0;
	// neither link is valid so only count p and end recursion
	else if ( !p->rLink && !p->lLink ) 
		return 1;
	else // recursively step down the tree counting leaves 
